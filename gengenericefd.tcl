...
proc writetoefd { topic firstvalue } {
global SQLREC TYPEFORMAT
  set flds [split $SQLREC($topic) ,]
  if { $firstvalue == 1 } {
      set record "if(iloop==0)\{	  sprintf(thequery,\"INSERT INTO [set topic] VALUES ("
  } elseif { $firstvalue == 2 } {
      set record "else if(iloop==numsamp-1)\{	  sprintf(auxquery,\" ("
  } else {
      set record "else\{\n	sprintf(auxquery,\" ("
  }
  set rformat ""
